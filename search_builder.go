...
func NewDefaultSearchBuilder(db *sql.DB, tableName string, modelType reflect.Type, options ...func(context.Context, interface{}) (interface{}, error)) *SearchBuilder {
	driver := getDriver(db)
	buildParam := getBuild(db)
	queryBuilder := NewDefaultQueryBuilder(tableName, modelType, driver, buildParam)
	return NewSearchBuilder(db, modelType, queryBuilder.BuildQuery, options...)
}
